...
Google TestModel root directory).  If your build system is not on this
list, you can take a look at `make/Makefile` to learn how Google TestModel
...
where `GTEST_ROOT` is the Google TestModel root directory).
...
and running Google TestModel tests, read some [samples](Samples.md), or continue with
[AdvancedGuide](V1_5_AdvancedGuide.md), which describes many more useful Google TestModel features.
