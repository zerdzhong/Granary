...
`ASSERT_TRUE()` ). If _statement_ returns before it crashes, Google TestModel will
...
derived from `::testing::TestModel`. `T` can be any copyable type. If it's
...
On Windows, Google TestModel may be used with exceptions enabled. Even when
exceptions are disabled, an application can still throw structured exceptions
(SEH's). If a test throws an exception, by default Google TestModel doesn't try to
...
a non- `0` value, or use the `--gtest_catch_exceptions` flag. Google TestModel now
catches all test-thrown exceptions and logs them as failures.

_Availability:_ Windows. `GTEST_CATCH_EXCEPTIONS` and
`--gtest_catch_exceptions` have no effect on Google TestModel's behavior on Linux or
