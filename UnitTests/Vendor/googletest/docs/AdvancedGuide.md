...
_Note_:
  ...
  * `key` must be a valid XML attribute name, and cannot conflict with the ones already used by Google TestModel (`name`, `status`, `time`, `classname`, `type_param`, and `value_param`).
